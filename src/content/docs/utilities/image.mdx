...
-   **`variant`**: _(optional)_ Determines the CSS class for image rendering. Defaults to `'w-auto h-100'`.

| Variant Value | Class Name                       |
| ------------- | -------------------------------- |
| `'cover'`     | `'w-100 h-100 object-fit-cover'` |
| _Default_     | `'w-auto h-100'`                 |
...
-   **`$attributes`**: _(array, passed by reference)_ The attributes for the image. This will be updated with default values if necessary.
...
-   **`$attributes`**: _(array)_ The attributes for the image.
-   **`$class`**: _(string, optional)_ Additional CSS classes for the image. Defaults to `"img-fluid w-100 object-fit-cover"`.
